...
import { ActionWrapper } from 'components/layout/action-wrapper'
...
import { ISSUPERADMIN, USERTOKEN } from 'app-constants'
...
import { getQuery } from '../action/helpers'
import { PAGENUMBER, PAGESIZE } from '../action/utils'
import { PageHeader } from '../../service-component/page-header'

const RolePage: FC = () => {
  const { getOrganization, organizations, state, action } = useGlobalContext()
  ...
  const { callRightSection, getAllRoles } = action

  const rightSectionProps = state?.global.rightSection

  const roleState = state?.role.getAllRoles
  const resLoading = state?.role.getAllRolesLoading
    ...
    <>
      <RightSection rsProps={rsProps}>
        ...
        {rsProps.isView('custom', 'view-role-actions') ? (
          <ViewActions states={state} actions={action} />
        ) : null}
      </RightSection>
      <div>
