...
export const useButtonRequest = (props: IProps) => {
  ...
  const isSuccess = () => {
    return (
      (update ? actionEnums.update : actionEnums.create) === adminAction &&
      http.httpStatus === 200
    )
  }
  ...
  const handleOnSubmit = () => {
    ...
    validateFormValues(
      required ? requiredState! : formState,
  ...
  const BTN = (
    <ButtonRequest
      title={update ? uTitle! : cTitle}
      isLoad={loading}
      isSuccess={isSuccess()}
      onClick={() => {
        update ? handleOnUpdate() : handleOnSubmit()
      }}
    />
  )
