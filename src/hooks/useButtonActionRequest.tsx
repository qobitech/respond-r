...
interface IProps {
  ...
  updateFormState: (
    title: string,
    callBack: (action: string, updatedFormState: object) => void
  ) => Promise<void>
...
export const useButtonActionRequest = (props: IProps) => {
  ...
  let isLoad = useStrictLoader(
    adminAction,
    ...buttonOptions.map((i) => i.action)
  )
  ...
  const handleOnSubmit = async (action: string, updatedFormState: object) => {
    ...
    validateFormValues(
      required ? requiredState! : updatedFormState,
      required ? requiredInitObj! : createInitObj,
      setFormError
    ).then((isValidated) => {
      if (isValidated) {
        if (otherValidations) {
  ...
  const handleOnUpdate = async (action: string, updatedFormState: object) => {
    ...
    validateFormValues(
      required ? requiredState! : updatedFormState,
      required ? requiredInitObj! : updateInitObj!,
      setFormError
    ).then((isValidated) => {
      if (isValidated) {
    ...
    return (
      <div style={{ width: "max-content", marginLeft: "10px" }}>
        <ButtonRequest
          ...
          onClick={() => {
            update
              ? typeof updateFormState === "function"
                ? updateFormState(title, handleOnUpdate)
                : void 0
              : typeof updateFormState === "function"
              ? updateFormState(title, handleOnSubmit)
              : void 0
          }}
        />
      </div>
  ...
  const BTNOption = () => {
    return (
      ...
      >
        {buttonOptions!.map((i) => (
          <BTNR title={i.title} key={i.id} action={i.action} />
        ))}
      </div>
    )
  }
